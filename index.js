...
app.post('/add_to_details',function(req,res){
   ...
   //var quantity = req.body.quantity;
   var image = req.body.image;
   var product = {Product_Id:Product_Id,Product_Name:Product_Name,Price:Price,image:image}
...
app.get('/track', function (req, res) {
   ...
   var sql = "select * from order1 where Order_Id LIKE '%" + Order_Id + "%' ";

   con.query(sql, [Order_Id], function (error, result) {
       if (error) console.log(error);
       res.render('order_tracking2', { order1: result });
   });
})
...
app.get('/order_read', function (req, res) {
   ...
   con.query(sql, function (error, result) {
       if (error) console.log(error);
       res.render('admin_or1', { order1: result })
   });
});
...
app.get('/update_order', function (req, res) {
   ...
   con.query(sql, [Order_Id], function (error, result) {
       res.render('admin_or2', { order1: result })
   });
});
...
app.post('/update_order', function (req, res) {
   ...
   var sql = "UPDATE order1 set Order_Details=?, Order_Status=? where Order_Id=?";

   con.query(sql, [Order_Details, Order_Status, Order_Id], function (error, result) {
       if (error) console.log(error);
       res.redirect('/order_read');
   });
});
